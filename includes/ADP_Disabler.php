...
class ADP_Disabler
{
    ...
    /**
     * Detect if the current request is a real admin backend request.
     *
     * is_admin() alone is not reliable because it returns true for ALL
     * AJAX requests (both frontend and admin) since they go through
     * wp-admin/admin-ajax.php.  This helper distinguishes the two by
     * checking the HTTP referer for AJAX calls — the same technique
     * ADP itself uses (Context::isDoingAjaxRefAdmin).
     *
     * @return bool
     */
    private function is_admin_request()
    {
        // Regular admin page (not AJAX) — definitely admin.
        if (is_admin() && ! wp_doing_ajax()) {
            return true;
        }

        // AJAX request: check whether the referer is a wp-admin page.
        if (wp_doing_ajax() && ! empty($_SERVER['HTTP_REFERER'])) {
            $referer  = wp_parse_url($_SERVER['HTTP_REFERER']);
            $admin    = wp_parse_url(admin_url());

            if (
                isset($referer['path'], $admin['path'])
                && strpos($referer['path'], $admin['path']) === 0
            ) {
                return true;
            }
        }
    ...
    {
        if (Helper::is_b2b_accepted_user() && ! $this->is_admin_request()) {
            return true;
        }
        return $value;
    }
