...
class Cart_Popup_Cross_Sell
{
     ...
     * Shows upsell products if available, otherwise falls back to category-based products.
     */
    private function get_cross_sell_html()
    {
        ...
        // First try to get upsell products for the current product
        ...
        if (! empty($upsell_ids)) {
            ...
            $available_upsells = array_diff($upsell_ids, $cart_product_ids);

            // Take up to 3 upsell products
            $product_ids = array_slice($available_upsells, 0, 3);
        }

        // If no upsell products available, fall back to category-based products
        ...
        // If still no products, return empty
        if (empty($product_ids)) {
            return '';
        }
...
<div class="justb2b-popup-cross-sell">
	...
	<div class="justb2b-cross-sell-products">
		<?php foreach ($product_ids as $cross_product_id) :
		    $cross_product = wc_get_product($cross_product_id);
		    if (! $cross_product) {
		        continue;
		    }
